...
contained on OpenChat DAppChain. Also is possible to create filter for indexed values.

## Filtering

Let's create a filter to get latest block created on OpenChat DAppChain and continuously print the block hash on console

```js
const {
  Client, CryptoUtils, OpenChatProvider
} = require('openchat-js')
...
// Instantiate web3 client using OpenChatProvider as provider
