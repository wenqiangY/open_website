...
In openchat.yml you need to add the following section
```yaml
HsmConfig:
  HsmEnabled: true
  HsmDevType: yubihsm
...
ID number of the private key on the HSM, we suggest to create this in advance. If you set this value to zero, OpenChat SDK will attempt to create a new private key on the HSM itself.
...
You should now have a key at with ID 100. Please set HsmSignKeyId to 100 in your openchat.yml
...
3. ./openchat call register_candidateV2 PUBLIC_KEY_IN_BASE64 10  --hsmconfig hsm.json -r http://dposv2.dappchains.com:80/query -w http://dposv2.dappchains.com:80/rpc  --chain dposv2
