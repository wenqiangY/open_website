...
## Activation and openchat.yaml

Activating the karma functionality is done from the openchat.yaml configuration file.
...
Example openchat.yaml fragment.
```yaml
KarmaEnabled: true
...
or from the command line the using `openchat` executable.
```bash
$ ./openchat karma --help
...
Use "openchat karma [command] --help" for more information about a command.
```
...
```
###### openchat karma update-oracle
The oracle can be updated using the `openchat update-oracle` command, which uses the UpdateOracle karma
method
```bash
$ openchat karma update-oracle --help
change the oracle

Usage:
  openchat karma update-oracle (new oracle) [old oracle] [flags]
...
```bash
$ karma update-oracle   default:0xAfaA41C81A316111fB0A4644B7a276c26bEA2C9F default:0xDeffe041cC978a193fCf0CE18b43b25B4592FC90  -k ./cmd/openchat/data/pri
oracle changed
```
...
###### openchat karma get-sources
Use the get-sources `openchat` command to list karma sources using the GetSources karma method.
```bash
$ openchat karma get-sources --help
list the karma sources

Usage:
  openchat karma get-sources [flags]
...
```
###### openchat karma reset-sources
The sources can be reset using the ResetSources karma method reached via the
`openchat karma update-sources` command. If you want to
add to existing sources will might want to use `openchat karma get-sources` to get those existing
sources.
```bash
$ openchat karma reset-sources --help
reset the sources

Usage:
  openchat karma reset-sources (oracle) [ [source] [reward] ]... [flags]
...
oracle.
```bash
$ openchat karma reset-sources default:0xDeffe041cC978a193fCf0CE18b43b25B4592FC90 "oauth" 3 "token" 5 "test" 7
sources successfully updated
```

##### go-openchat update sources
The following Go code fragment gives an example of how you might do this in Go using go-openchat,
error checking skipped for readability.
```go
import `github.com/openchatnetwork/go-openchat/builtin/types/karma`

func AddSource(name string, reward int64, signer auth.Signer, oracle openchat.Address, karmaContact client.Contract) {
...
###### openchat karma append-sources-for-user
New sources can be associated with a user using karma method AppendSourcesForUser, this can
be accessed with the `openchat karma append-sources-for-user` command.
```bash
$ ./openchat karma append-sources-for-user  --help
add new source of karma to a user, requires oracle verification

Usage:
  openchat karma append-sources-for-user (user) (oracle) [ [source] [count] ]... [flags]
...
sources.
```bash
$ ./openchat karma append-sources-for-user default:0xAfaA41C81A316111fB0A4644B7a276c26bEA2C9F  default:0xDeffe041cC978a193fCf0CE18b43b25B4592FC90 "oauth" 4 "token" 3 -k ./cmd/openchat/data/pri
sources successfully updated
```

###### openchat karma delete-sources-for-user
Existing sources can be disassociated with a user using the karma method DeleteUsersForUser.
For this you can use the `openchat karma delete-sources-for-user`.
```bash
$ openchat karma delete-sources-for-user --help
delete sources assigned to user, requires oracle verification

Usage:
  openchat karma delete-sources-for-user (user) (oracle) [name]... [flags]
...
```bash
$ ./openchat karma delete-sources-for-user default:0xAfaA41C81A316111fB0A4644B7a276c26bEA2C9F  default:0xDeffe041cC978a193fCf0CE18b43b25B4592FC90 "oauth" "token" -k ./cmd/openchat/data/pri
sources successfully deleted
```
###### openchat karma get-user-state
To get the list of sources that have been associated with a user, you can use the openchat
command `openchat karma get-user-state`.
```bash
$ openchat karma get-user-state --help
list the karma sources for user

Usage:
  openchat karma get-user-state (user address) [flags]
...
```bash
./openchat karma get-user-state default:0xDeffe041cC978a193fCf0CE18b43b25B4592FC90
```
