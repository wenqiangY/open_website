...
```shell
openchat genkey -a pubkey -k privkey
```
...
To send transactions to the network we can use the example-cli from the [go-openchat project](https://github.com/openchat/go-openchat). This can be built by running
```shell
make example-cli
```
