...
This documentation describes how to run openchat in a multi node setup.
...
1. Download the binaries:
    ```bash
    wget https://private.openchat.co/openchat/linux/build-208/openchat
    chmod +x openchat
    ```
...
First, we need to get node keys from each node. Go to the working directory and run `openchat nodekey`:
...
To run openchat, we need to tell each node about its peers. The general format is:

```bash
openchat run --persistent-peers tcp://<node1_key>@<node1_ip>:46656,tcp://<node2_key>@<node2_ip>:46656,...tcp://<nodeN_key>@<nodeN_ip>:46656
```
...
```bash
openchat run --persistent-peers tcp://e728bada822af677b95cb8ff126ca72cc4e3dc74@10.6.7.8:46656,tcp://4953e5726664985cc1cc92ae2edcfc6e089ba50d@10.3.2.1:46656,tcp://02c90b57d241c3c014755ecb07e0c0d232e07fff@10.7.6.5:46656
```

On node 2:

```bash
openchat run --persistent-peers tcp://47cd3e4cc27ac621ff8bc59b776fa228adab827e@10.2.3.4:46656,tcp://4953e5726664985cc1cc92ae2edcfc6e089ba50d@10.3.2.1:46656,tcp://02c90b57d241c3c014755ecb07e0c0d232e07fff@10.7.6.5:46656
```
...
Notice `ExecStart`, it is constructed using the same concept from the previous section when running openchat directly. This means each node has a different startup script.
...
ExecStart=/home/ubuntu/openchat run --persistent-peers tcp://e728bada822af677b95cb8ff126ca72cc4e3dc74@10.6.7.8:46656,tcp://4953e5726664985cc1cc92ae2edcfc6e089ba50d@10.3.2.1:46656,tcp://02c90b57d241c3c014755ecb07e0c0d232e07fff@10.7.6.5:46656
Restart=always
RestartSec=2
...
Save it to `/etc/systemd/system/openchat.service`. Run these to activate it:

```bash
sudo systemctl daemon-reload
sudo systemctl start openchat.service
```

You may now inspect the output using:

```bash
sudo journalctl -u openchat.service
```
...
```bash
sudo systemctl enable openchat.service
```
...
tcp6       0      0 :::46656                :::*                    LISTEN      2135/openchat
tcp6       0      0 :::46657                :::*                    LISTEN      2135/openchat
tcp6       0      0 :::46658                :::*                    LISTEN      2135/openchat
```
...
The playbook is available [here](https://github.com/openchat/openchat-playbooks/blob/master/openchat-playbook.yml)
...
```bash
ansible-playbook -i inventory.yml -vv openchat-playbook.yml
```
